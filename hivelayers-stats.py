...
def hivecommunity():
    ...
    d = st.date_input(
         ...
         max_value=datetime.date(2021, 6, 13))
...
def get_chart(df_user_details,token,sym_list,sym):
    ...
    if(token):
        if sym!='All':  # Then a particular symbol is selected in the selectbox .
            ...
            st.write('<div class="card"><div class="card-header"><center>Total '+sym+' from past 30 days  : '+ '%.6f' % sum_sym+' '+sym+' , In HIVE = '+'%.6f' % total +'.</center>',unsafe_allow_html=True)
...
def hivetoken():
    ...
    token=st_select_token.selectbox('Select the token you wish to see dividends for',['UTOPIS'])
    #,'BRO','INDEX','EDS','SPI','TAN','DHEDGE'
    if token:
        start=dt.now()
